...
每次决策同时输入可见局面和全部合法候选动作。共用的 MLP 根据全局表示 `h = X[0]`，对 `[o, h, o ⊙ h]` 评分，将动作、状态及两者的交互结合起来。
...
| 每个候选动作 | 60 + 3 个 ID | 动作类型、卡牌、目标及招式 |
...
以给蜜集大蛇附能为例，它的场上位置 token 由 32 个状态数值与四份 64 维卡牌表示组合而成，分别对应宝可梦、第一张道具和前两张能量卡。拼接得到的 288 维再投影到 384 维。附能候选动作则组合 60 维动作特征、28 维试探特征，以及卡牌、目标和招式表示。试探器可以标记附能后新增的合法攻击；交叉注意力读取整个局面，再由策略将这个附能动作与其他动作比较评分。

试算使用固定方式补全隐藏区域，因此依赖隐藏卡牌身份的效果仍是近似结果。试探器不规划对手的应对；抽牌或检索之后，选项数量特征会被屏蔽。
