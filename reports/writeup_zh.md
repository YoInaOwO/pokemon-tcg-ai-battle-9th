...
试探器让策略根据动作的即时后果比较不同选择。对符合条件的主要阶段单选决策，它在临时引擎状态中分别执行最多 64 个候选动作，并在固定上限内继续处理强制后续动作和抛硬币分支。它为每个候选动作追加 28 维特征，包括伤害、击倒、获得的奖赏、手牌和牌库数量变化，以及合法攻击的变化。

例如，附能可以让蜜集大蛇的攻击变得合法。引擎报告这个变化，策略再决定现在攻击，还是先用另一个特性。试算使用固定方式补全隐藏区域，因此依赖隐藏卡牌身份的效果仍是近似结果。试探器不规划对手的应对；抽牌或检索之后，选项数量特征会被屏蔽。
