...
**TL;DR。** 两次最终提交使用完全相同的牌表和模型权重。有限引擎前瞻将动作后果提供给约 780 万参数的策略网络，由网络学习行动顺序和资源分配。我先进行 behavioural cloning 和 value fine-tune，再用按竞技场构建的对手池训练 PPO。智能体最终取得第九名。
...
例如，附加一张能量可能让蜜集大蛇的攻击变为合法。试探器把这个变化交给网络，策略仍要判断：现在就攻击，还是先用另一个特性，把伤害推到击倒线。
...
在评分差不超过 200 的对局中，面对完全相同的牌表，我取得了 **74 场 53 胜，即 71.6% [60.5%–80.6%]**。相同牌表有助于评估策略水平，不过对手水平仍有差异。
