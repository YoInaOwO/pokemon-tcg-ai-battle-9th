...
引擎需要完整状态才能运行。试探器根据我的牌表重建己方未见卡牌，使用固定随机种子洗牌，并用占位的能量和基础宝可梦填充对手的隐藏区域。这些牌只是占位，不是在预测对手的真实卡牌。试探器提取伤害、奖赏变化等直接后果，不规划对手的应对。若某个效果依赖隐藏卡牌的身份，试探结果仍可能不准确。抽牌或检索之后，选项数量特征会被屏蔽，避免结果依赖重建牌库时设定的牌序。
