...
模型为每个合法选项生成表示，输入包括动作特征及其对应的卡牌、目标和招式。选项再通过交叉注意力读取状态。例如，指向某个备战区位置的动作，可以读取该宝可梦当前的 HP 和能量。首个编码 token 作为全局表示 `h = X[0]`。MLP 对 `[o, h, o ⊙ h]` 评分，将动作、状态及两者的交互结合起来。
...
我先编码卡牌，再将卡牌表示与当前局面结合。全部 1,267 张卡都使用同一种格式的 207 维静态特征，不适用的字段填零。
...
我将卡牌表示与当前 HP、能量及其他局面信息结合：

| 局面信息 | 编码方式 |
|---|---|
| 道具 | 是否携带、数量，以及第一张附着道具的卡牌表示 |
| 进化 | 当前卡牌的进化阶段、下方卡牌数量，以及是否本回合登场 |
| 卡牌区域 | 用卡牌 ID、掩码和数量表示手牌、已展示卡牌、弃牌及己方未见卡牌（牌库与未翻开的奖赏卡） |
| 全局状态与历史 | 先后手、资源、动作使用情况、近期事件及记忆中的公开卡牌 |
| 决策与候选动作 | 提示、选择数量限制、动作类型、卡牌、目标及招式 |

Opponent belief 用来估计对手的卡组流派。我将对手已公开的卡牌与回放牌表匹配，按牌表频率和重合程度加权，再汇总成 14 类流派加“其他”类别的概率。BC 期间，dropout 偶尔将这个分布替换为“未知”。

引擎试探器额外提供 28 维特征，描述动作的即时后果。对符合条件的主要阶段单选决策，它最多试执行 64 个候选，并在有限范围内继续处理强制后续动作和抛硬币分支。例如，它能告诉模型，附能后蜜集大蛇的攻击变得合法；策略再决定直接攻击，还是先用另一个特性。

引擎需要完整的隐藏区域才能进行这些试算。我根据牌表重建己方未见卡牌，使用固定随机种子洗牌；对手的隐藏区域则用占位牌填充，不预测实际卡牌的身份。试探器计算即时效果，不规划对手的应对。依赖隐藏卡牌身份的效果仍可能不准确。抽牌或检索之后，选项数量特征会被屏蔽，避免结果依赖重建牌库时设定的牌序。
