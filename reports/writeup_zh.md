...
**TL;DR。** 两次最终提交使用完全相同的牌表和模型权重。一个约 780 万参数的网络结合场面信息和短程引擎试探，为合法动作评分。我先进行 behavioural cloning 和 value fine-tune，再用按竞技场构建的对手池训练 PPO。最终 PPO 运行每次更新采集 839 万次决策，耗时 56.3 小时，智能体最终取得第九名。
...
所有候选动作共用同一个评分网络。因此，无论提示中有多少个选项，模型都能逐一评分，并将从其他动作中学到的经验用于当前选择。
...
我将资源、公开历史和动作后果直接编码为特征。下表分别列出数值特征的维度和 ID。
...
Opponent belief 指根据对手已公开的卡牌，估计对方属于哪种卡组流派。我将这些卡牌与回放中的牌表匹配，按牌表出现频率和卡牌重合程度加权，再按流派汇总并归一化，得到概率分布，作为全局状态输入网络。克隆训练中的 dropout 会偶尔将这项判断替换为“未知”，减少策略对卡组识别的依赖。
...
试探器按固定方式补全隐藏区域，不读取对手真实手牌。抽牌或检索之后，部分选项数量特征会被屏蔽，避免结果依赖补全牌库的牌序。这些特征描述动作的短程后果，不预测对手的完整应对。

## 4. 训练

### 第一步：behavioural cloning

我用 7 月 14 日至 8 月 12 日收集的约 13.6 万场回放建立训练集，先跨流派训练一个共享模型，再针对各流派微调。附能、进化等共通决策可以跨卡组学习，为示范数据较少的卡组提供训练起点。

克隆训练中，获胜方决策的权重为 1.0，落败方为 0.3，同时使用十天尺度的时间衰减，并提高较强选手样本的权重。

### 第二步：value fine-tune

进入 PPO 前，我用起始策略生成对局，保持策略参数不变，用对局结果拟合两个 critic，使价值估计适应智能体实际遇到的状态。PPO 根据回报与预测价值之间的差异更新策略；先校准 critic，能为这些更新提供更合适的比较基准。
...
| 对手组成 | 作用 |
|---|---|
| 按竞技场出现频率加权的前 40 份牌表，由对应流派的克隆模型操作 | 接近竞技场的卡组分布 |
| 变异牌表 | 由对应的克隆模型操作，练习应对调整过的卡牌组合 |
| 主办方提供的四种初始脚本 | 覆盖简单但不同的打法，以应对天梯中 10% 的随机对手匹配 |
| 自身镜像 | 从双方视角练习镜像对局 |

构建对手池时，这四部分的对局比例依次设为 75%、10%、10% 和 5%。镜像对局会同时采集双方的轨迹，因此它在训练样本中所占的比例高于对局比例。

变异牌表从各流派最常见的牌表出发，执行 5–10 次换牌操作。替换牌取自该流派已有的卡牌，数量上限采用该流派任一已观察牌表中的最大张数。生成的牌表包含 60 张卡，通过引擎验证后才加入对手池。

PPO 包含裁剪策略更新、价值回归和熵奖励。裁剪抑制策略的突然变化，价值回归改善回报估计，熵奖励则鼓励策略继续探索其他动作。广义优势估计由 oracle critic 提供。我使用 γ = 0.997、λ = 0.95、裁剪范围 0.2、学习率 1e-4。奖赏进度塑形和限制策略偏离克隆模型的惩罚，在八次更新内退火到零，使最终目标集中于赢下对局。

**为什么 rollout 要这么大？** 我先把每次更新前采集的决策数从 13.1 万提高到 52.4 万，发现训练胜率的上限提高了。这个结果促使我直接将 rollout 提到 839 万，是前一次的十六倍。大 rollout 运行起初提升较慢，但随后持续达到了更高的分数。

小 rollout 中，稀有对局只有少量样本，一次走运的开局就可能对更新产生较大影响。增大 rollout 后，每次更新前都能采集更多对局，覆盖先手和后手。平均绝对优势值从最初的 0.35，下降到接近最高训练分数时的 0.16。我的理解是，更充分的采样有助于从对局的随机波动中辨认这些较小的信号。代价是同样决策总量下，策略更新次数更少，这也能部分解释初期进步较慢的现象。

![训练进展与先后手](report_training_evidence.png)

*图 2：左侧是按竞技场频率加权的训练分数，每个固定克隆对手使用最近至多 300 场的滚动窗口，不含脚本、变异和镜像。右侧是第 48 次 rollout 中先后手分别的表现，n 为两种顺序的合计场数。平局按半胜计。*

最终运行使用 8 张 RTX 4090 GPU，在 56.3 小时内完成 57 次更新。训练分数从 52.4% 升至第 48 次更新时的 83.3%，此时累计约 4.03 亿次决策；第 45–57 次更新保持在 80.5%–83.3%。对胡地时，先手为 63.5%，后手为 50.6%；对多龙巴鲁托则分别为 83.9% 和 83.7%。
...
*图 3：此处“Elo-matched”指评分差不超过 200。图中展示至少 15 场的分组，省略的对局仍计入总体。“Mirror”包含所有蜜集大蛇牌表。灰色线段为近似 Wilson 95% 区间；重复遇到同一对手可能使区间偏乐观。*

在这一分组中，面对完全相同的牌表，我取得了 **74 场 53 胜，即 71.6% [60.5%–80.6%]**。双方卡牌相同，这些对局更能反映策略如何使用卡组，不过对手水平仍有差异。

面对多龙巴鲁托的 Jamming Tower 构筑，我的胜率为 55.4%，对 Risky Ruins 构筑则为 42.9%。对 Espeon–Sylveon 的 32 场仅为 12.5%：即使带了非 ex 攻击手，这条路线仍不够可靠。对胡地的 138 场，我的胜率为 45.7%，说明真实选手比训练中的克隆对手更难应付。

## 6. 试过但没有奏效的方法

**MCTS。** 我尝试了对隐藏状态采样、以策略作为先验、以 critic 评估叶节点的 MCTS，但没有获得优于直接策略的有效提升。在较短的行动预算内，搜索必须把算力分到多个隐藏状态样本；每棵树又会按其采样状态已经确定来规划。最终提交保留了用于特征的有限引擎试探。

**更大的网络。** 增加容量也没有带来提升。网络已经能读取详细的动作后果，因此我选择把预算花在更多训练经验上。

## 7. 可改进的地方

我会用共享 GPU 服务合并多场对局的决策请求，批量推理。推理更快，就能在同样的预算内采集更多训练经验。

我会更早为第二套卡组留出时间，并给第一套卡组设定明确的训练预算，确保提交前还有时间训练和评估另一套方案。
