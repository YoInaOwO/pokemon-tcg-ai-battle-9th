...
蜜集大蛇的 Syrup Storm 计算己方全场的草能量，厄诡椪的 Myriad Leaf Shower 则计算双方战斗宝可梦身上的能量。多铺几个厄诡椪可以增加抽牌和附能机会，但也要给两条进化路线留出备战区位置。
...
模型将局面输入分组表示为 64 个 token，再交给宽度 384、六个注意力头的五层 Transformer 编码。

类型与位置嵌入用于区分区域和位置；填充掩码让注意力跳过空的手牌和展示牌位置。

共用的 MLP 根据候选动作表示、编码后的第一个状态 token，以及这两个向量对应位置相乘的结果，为每个候选动作评分。
...
效果字段通过规则解析卡牌文本得到，再应用已保存的修正。模型将每张卡的特征投影成 64 维向量，再加上为这张卡单独学习的向量，即使效果特征相同也能保留不同卡牌的身份。训练时，ID dropout 会偶尔去掉这个独立向量。
...
静态特征描述卡牌有哪些效果，但策略还需要知道这些效果在当前局面中意味着什么。引擎前瞻直接提供这些后果，减少网络仅凭回放样本学习规则相互作用的负担。

对符合条件的主要阶段单选决策，我在不同的临时引擎状态中试执行最多 64 个候选动作。试探器将执行结果与初始局面比较，继续处理强制后续动作，并对抛硬币结果展开分支。遇到需要玩家选择的节点、对局结束或达到固定计算上限时，展开停止。

得到的 28 维特征覆盖即时伤害、击倒、奖赏与手牌及牌库变化，各分支按概率加权的结果，以及新增合法攻击。训练和实际对战时，这些特征都进入动作表示。策略学习在选择动作时，将这些后果与展开和资源需求一起权衡。
...
我使用 8 月 3–12 日的回放训练 BC。通用模型先学习各流派、双方玩家在回放中作出的决策。随后，各流派模型都从这个通用模型的权重初始化，只使用操控对应流派时的决策数据，以较低学习率微调整个网络。

克隆训练中，获胜方决策的权重为 1.0，落败方为 0.3，同时提高较新对局和较强选手样本的权重。

### 第二步：value fine-tune

我用经过流派微调的 Ogerpon–Hydrapple BC 模型生成对局，保持策略参数不变，用对局结果拟合两个 critic。随后，PPO 从这个完成 value fine-tune 的模型开始训练。
...
终局奖励为胜利 +1、失败 −1、平局 0。我使用 γ = 0.997、λ = 0.95、裁剪参数 0.2、学习率 1e-4。裁剪限制策略变化，价值回归拟合回报，熵奖励鼓励探索。广义优势估计由 oracle critic 提供。奖赏进度塑形和限制策略偏离克隆模型的惩罚，在八次更新内退火到零。
...
对 Espeon–Sylveon 的 32 场，我的胜率为 12.5%。仙子伊布的 Safeguard 能挡住两个主力 ex 的攻击伤害。大竺葵和卡璞·哞哞可以绕过这项防护，但太阳伊布的 Psych Out 又足以将它们从满血击倒。
